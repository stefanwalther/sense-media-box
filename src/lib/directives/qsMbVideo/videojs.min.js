...
(function () {
	...
	s.Aa = function () {
		...
		if ( c ) {
			for ( var d = c.length - 1; 0 <= d; d-- ) {
				a.U( new t.tb( this.j(), {rate: c[d] + "x"} ) );
			}
		}
		return a
	};
	...
	t.g = t.p.extend( {
		i: function ( a, c, d ) {
			t.p.call( this, a, c, d );
			for ( d = t.g.qb.length - 1; 0 <= d; d-- ) {
				this.c( t.g.qb[d], this.Fd );
			}
	...
	s.e = function () {
		...
		if ( !c || this.movingMediaElementInDOM === l ) {
			c ? (d = c.cloneNode( l ), t.g.Gb( c ), c = d, a.I = k) : (c = t.e( "video" ), d = videojs.W.pa( {}, a.se ), (!t.wb || a.options().nativeControlsForTouch !== f) && delete d.controls, t.Uc( c, t.h.z( d, {
				id: a.id() + "_html5_api",
				"class": "vjs-tech"
	...
	s.Wb = function ( a ) {try {this.b.currentTime = a} catch ( c ) {t.log( c, "Video is not ready. (Video.js)" )}};
	...
	t.g.isSupported = function () {
		try {t.A.volume = 0.5} catch ( a ) {return l}
	...
	t.f = t.p.extend( {
		i: function ( a, c, d ) {
			...
			var e = c.source;
			d = c.parentEl;
			var g = this.b = t.e( "div", {id: a.id() + "_temp_flash"} ), h = a.id() + "_flash_api", j = a.m, j = t.h.z( {
				...
				loop: j.loop,
				muted: j.muted
			}, c.flashVars ), p = t.h.z( {wmode: "opaque", bgcolor: "#000000"}, c.params ), h = t.h.z( {
				id: h,
				name: h,
				"class": "vjs-tech"
			}, c.attributes );
	...
	t.f.version = function () {
		var a = "0,0,0";
		try {a = (new window.ActiveXObject( "ShockwaveFlash.ShockwaveFlash" )).GetVariable( "$version" ).replace( /\D+/g, "," ).match( /^,?(.+),?$/ )[1]} catch ( c ) {try {navigator.mimeTypes["application/x-shockwave-flash"].enabledPlugin && (a = (navigator.plugins["Shockwave Flash 2.0"] || navigator.plugins["Shockwave Flash"]).description.replace( /\D+/g, "," ).match( /^,?(.+),?$/ )[1])} catch ( d ) {}}
		return a.split( "," )
	};
	...
	function Y ( a, c, d ) {
		for ( var e = a.Ma, g = 0, h = e.length, j, p; g < h; g++ ) {
	...
	s.load = function () {
		0 === this.sa && (this.sa = 1, t.ye( this.Xc, t.bind( this, function ( a, c, d ) {
			if ( a ) {
				this.error = a, this.sa = 3, this.l( "error" );
			} else {
				var e, g;
				a = d.split( "\n" );
				c = "";
		...
		if ( 0 < this.ca.length ) {
			...
			if ( this.Ub === b || a < this.Ub || this.cb <= a ) {
				...
				for ( ; ; ) {
					q = c[w];
					if ( q.Ca <= a ) {
						e = Math.max( e, q.Ca ), q.Ua && (q.Ua = l);
					} else if ( a < q.startTime ) {
						if ( d = Math.min( d, q.startTime ), q.Ua && (q.Ua = l), !g ) {
							break
						}
					} else {
	...
	t.sb = t.aa.extend( {
		i: function ( a, c ) {
			c.track = {
				M: function () {return c.kind},
				j: a,
				label: function () {return c.kind + " off"},
				Fb: r( l ),
				mode: r( l )
			};
			t.aa.call( this, a, c );
	...
	t.T.prototype.za = function () {
		...
		for ( var d = 0; d < this.d.textTracks().length; d++ ) {
			c = this.d.textTracks()[d], c.M() === this.G && a.push( new t.aa( this.d, {track: c} ) );
		}
		return a
	};
	t.Oa = t.T.extend( {
